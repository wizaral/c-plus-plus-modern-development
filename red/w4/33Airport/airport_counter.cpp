...
class AirportCounter {
    array<size_t, static_cast<size_t>(TAirport::Last_)> arr;
public:
    AirportCounter() {
        arr.fill(0);
    }
    ...
    AirportCounter(TIterator begin, TIterator end) {
        arr.fill(0);
        for (; begin != end; ++begin)
            ++arr[static_cast<size_t>(*begin)];
    }

    size_t Get(TAirport airport) const {
        return arr[static_cast<size_t>(airport)];
    }

    void Insert(TAirport airport) {
        ++arr[static_cast<size_t>(airport)];
    }
    void EraseOne(TAirport airport) {
        --arr[static_cast<size_t>(airport)];
    }
    void EraseAll(TAirport airport) {
        arr[static_cast<size_t>(airport)] = 0;
    }
    ...
    Items GetItems() const {
        ...
        for (size_t i = 0, size = arr.size(); i < size; ++i)
            items[i] = {static_cast<TAirport>(i), arr[i]};
        return items;
    }
};
